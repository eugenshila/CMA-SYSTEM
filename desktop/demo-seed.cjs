/**
 * Realistic demonstration data for a parish CMA — St. Joseph Mukasa Parish,
 * Kahawa West, with the St. Peter and Paul (Marengeta) and St. Francis of
 * Assisi (Soweto) sub-parishes. Shared by two entry points:
 *
 *   1. the desktop build — desktop/bootstrap.cjs seeds this dataset on the
 *      first launch of a fresh install (disable with CMA_DEMO_SEED=0), and
 *   2. the development database seed — `npm run db:seed` (scripts/seed.ts is
 *      a thin wrapper around this file).
 *
 * Plain CommonJS on `pg` + `bcryptjs` so the packaged Electron main process
 * can run it in-process. The dataset is deterministic (seeded PRNG),
 * idempotent-safe (it refuses to run on a database that already has members
 * or users unless `force` is set) and writes everything with batched
 * multi-row INSERTs so it runs in a couple of seconds.
 */
'use strict';
...
const ts = (y, m, d, hh = 10, mm = 0) => `${date(y, m, d)} ${String(hh).padStart(2, '0')}:${String(mm).padStart(2, '0')}:00+03`;

/* ------------------------------------------------------------------ *
 * reference content
 * ------------------------------------------------------------------ */
const FIRST = [
  'James','John','Robert','Michael','William','David','Richard','Thomas','Daniel','Matthew','Andrew','Samuel','Joseph','Peter','Paul',
  'Mark','Stephen','Nicholas','Gabriel','Raphael','Emmanuel','Vincent','Martin','Gregory','Bernard','Francis','Anthony','Charles','Edward','Henry',
  'George','Alexander','Nathan','Adrian','Marco','Stefan','Andre','Miguel','Tomas','Erik','Lars','Pawel','Ivan','Oliver','Felix',
  'Oscar','Hugo','Simon','Patrick','Brendan','Kevin','Sean','Liam','Declan','Timothy','Victor','Walter','Dominic','Frederick','Clifford',
];
const LAST = [
  'Smith','Johnson','Brown','Taylor','Miller','Wilson','Anderson','Jackson','White','Harris','Martin','Thompson','Garcia','Martinez',
  'Clark','Rodriguez','Lewis','Walker','Hall','Allen','Young','King','Wright','Scott','Green','Baker','Adams','Nelson',
  'Carter','Mitchell','Turner','Phillips','Campbell','Parker','Evans','Edwards','Collins','Stewart','Morris','Murphy','Sullivan','Delaney',
  'Rossi','Moretti','Kowalski','Novak','Mueller','Fischer','Weber','Meyer','Costa','Silva','Santos','Ferreira','Pereira','Bennett',
];
const OCCUPATIONS = [
  ...
  'Mason','Warehouse Supervisor','Sales Manager','Agronomist','Doctor','Clerk','Tailor','Delivery Rider','Contractor','Catechist',
];
const AREAS = ['Kahawa West','Marengeta','Soweto','Northgate','Brookside','Hillcrest','Riverdale','Greenpark','Lakeside','Morningside','Fairview','Springfield','Westbrook','Oakwood'];
const SCC = ['St. Joseph SCC','Holy Family SCC','St. Jude SCC','Good Shepherd SCC','St. Peter SCC','St. Anthony SCC','St. Francis SCC','St. Michael SCC'];
...
async function flush(client, table, chunk = 200) {
...
async function setval(client, table) {
...
async function seedDemoData(options = {}) {
  const {
    ...
    encryptionKey = process.env.ENCRYPTION_KEY || 'cma-dev-encryption-key-change-me-64hex00',
    referenceSqlPath = path.resolve(__dirname, '..', 'db', 'migrations', '002_reference_data.sql'),
    logger = console,
  } = options;

  if (!databaseUrl) throw new Error('seedDemoData: databaseUrl is required');

  // Identical format to src/lib/crypto.ts: AES-256-GCM with sha256(ENCRYPTION_KEY).
  const aesKey = crypto.createHash('sha256').update(String(encryptionKey)).digest();
  ...
  const blindIndex = (plain) =>
    plain ? crypto.createHmac('sha256', aesKey).update(String(plain).trim().toUpperCase()).digest('hex') : null;

  // Placeholder document files, mirroring src/lib/files.ts storeFile.
  const UPLOAD_ROOT = uploadRoot;

  const client = new pg.Client({ connectionString: databaseUrl });
  await client.connect();
  logger.log('[demo-seed] connected');

  const existing = await client.query(
    'SELECT (SELECT count(*)::int FROM members) AS members, (SELECT count(*)::int FROM users) AS users',
  );
  const hasData = existing.rows[0].members > 0 || existing.rows[0].users > 0;
  if (hasData && !force) {
    logger.log(
      `[demo-seed] database already contains ${existing.rows[0].members} members / ${existing.rows[0].users} users — skipping.`,
    );
    await client.end();
    return false;
  }
  ...
  if (hasData && force) {
    logger.log('[demo-seed] force: truncating demo tables…');
    ...
    const refSql = await fs.readFile(referenceSqlPath, 'utf8');
    await client.query(refSql);
    logger.log('[demo-seed] reference data re-applied (loan_types, contribution_types, system_settings)');
  }
  ...
  dioceses.add({ id: 1, country_id: countryId, code: 'ADN', name: 'Archdiocese of Nairobi', type: 'archdiocese', bishop: 'Most Rev. Gregory O’Connell' });
  dioceses.add({ id: 2, country_id: countryId, code: 'MUR', name: 'Diocese of Muranga', type: 'diocese', bishop: 'Most Rev. Anthony Gallagher' });

  const deaneries = new Table('deaneries', ['id', 'diocese_id', 'code', 'name']);
  deaneries.add({ id: 1, diocese_id: 1, code: 'DEAN-KHW', name: 'Kahawa West Deanery' });
  ...
  parishes.add({ id: 1, deanery_id: 1, diocese_id: 1, code: 'SJM', name: 'St. Joseph Mukasa Parish, Kahawa West', parish_priest: 'Fr. Richard Sullivan', cma_chaplain: 'Fr. Thomas Brennan', address: 'P.O. Box 1234-00509, Kahawa West, Nairobi', phone: '+254712000001', email: 'office@stjosephmukasa.or.ke' });

  const churches = new Table('churches', ['id', 'parish_id', 'code', 'name', 'type', 'location']);
  const churchList = [
    { id: 1, parish_id: 1, code: 'SJM-MAIN', name: 'St. Joseph Mukasa Parish Church', type: 'parish_church', location: 'Kahawa West' },
    { id: 2, parish_id: 1, code: 'SJM-MRG', name: 'St. Peter and Paul Sub-Parish, Marengeta', type: 'outstation', location: 'Marengeta' },
    { id: 3, parish_id: 1, code: 'SJM-SOW', name: 'St. Francis of Assisi Sub-Parish, Soweto', type: 'outstation', location: 'Soweto' },
  ];
  ...
  const sccRows = [];
  let sccId = 0;
  for (const c of churchList.slice(0, 3)) {
    for (let i = 0; i < 2; i++) {
      sccId++;
  ...
  for (let i = 1; i <= MEMBER_COUNT; i++) {
    ...
    // all demo members belong to the single demo parish
    const parishId = 1;
    ...
    memberRows.push({
      ...
      employer: chance(0.5) ? pick(['Self-employed', 'Ministry of Education', 'County Government', 'Halcyon Telecom', 'National Power Co.', 'First Equity Bank', 'Census Bureau', 'Private Practice']) : null,
  ...
  const staff = [
    { id: 1, role: 'super_admin', name: 'Mr. Gregory Simmons', email: 'superadmin@cma.or.ke', phone: '254700000001', login: 'SUPER001', pw: 'Cma@Super2026', parish: null },
    { id: 2, role: 'admin', name: 'Mr. Daniel Whitfield', email: 'admin@stjosephmukasa.or.ke', phone: '254700000002', login: 'ADMIN001', pw: 'Cma@Admin2026', parish: 1 },
    { id: 3, role: 'chairman', name: 'Mr. Robert Ashford', email: 'chairman@stjosephmukasa.or.ke', phone: '254700000003', login: 'CHAIR001', pw: 'Cma@Chair2026', parish: 1 },
    { id: 4, role: 'treasurer', name: 'Mr. Edward Callahan', email: 'treasurer@stjosephmukasa.or.ke', phone: '254700000004', login: 'TRES001', pw: 'Cma@Treas2026', parish: 1 },
    { id: 5, role: 'secretary', name: 'Mr. Samuel Rossi', email: 'secretary@stjosephmukasa.or.ke', phone: '254700000005', login: 'SEC001', pw: 'Cma@Sec2026', parish: 1 },
    { id: 6, role: 'sacco_officer', name: 'Mr. Vincent Hartley', email: 'sacco@stjosephmukasa.or.ke', phone: '254700000006', login: 'SDP001', pw: 'Cma@Sacco2026', parish: 1 },
    { id: 7, role: 'loan_committee', name: 'Mr. Francis Delaney', email: 'loans@stjosephmukasa.or.ke', phone: '254700000007', login: 'LOAN001', pw: 'Cma@Loan2026', parish: 1 },
    { id: 8, role: 'auditor', name: 'Mr. Bernard Whitaker', email: 'auditor@cma.or.ke', phone: '254700000008', login: 'AUD001', pw: 'Cma@Audit2026', parish: null },
  ];
  staff.forEach((s) =>
    users.add({
      id: s.id,
  ...
  const nextReceiptNo = (year, month) => {
  ...
  function addPayment(opts) {
    ...
    receipts.add({
      ...
      parish_name: 'St. Joseph Mukasa Parish, Kahawa West',
      category: opts.category,
  ...
  /* ---------------- Last Respect Insurance premiums ---------------- */
  // Enrolled members pay a KSh 200 monthly premium; the scheme pays the
  // enrolled member's family a cash benefit towards funeral and end-of-life
  // expenses (see FUN/2026/0003 above), so loved ones are not burdened
  // during difficult times.
  const LRI = ctId('last_respect_insurance');
  if (LRI) {
    const enrolled = activeMembers.filter((m, idx) => idx % 10 !== 0); // ~90% enrolled
    for (const period of periods.slice(2)) {
      const [y, m] = period.split('-').map(Number);
      for (const member of enrolled) {
        mcId++;
        const recentPremium = m >= 7 && y === 2026;
        let premiumPaid = 200;
        let premiumStatus = 'paid';
        if (recentPremium && member.id % 7 === 0) {
          premiumPaid = 0;
          premiumStatus = m === 8 && y === 2026 ? 'unpaid' : 'overdue';
        } else if (recentPremium && member.id % 13 === 0) {
          premiumPaid = 100;
          premiumStatus = 'partial';
        }
        memberContributions.add({
          id: mcId,
          member_id: member.id,
          contribution_type_id: LRI,
          financial_year_id: y === 2025 ? 1 : 2,
          period,
          period_label: PERIOD_LABEL[period],
          amount_due: 200,
          amount_paid: premiumPaid,
          penalty: 0,
        ...
        if (premiumPaid > 0) {
          const method = chance(0.6) ? 'mpesa' : 'cash';
          addPayment({
            memberId: member.id,
            amount: premiumPaid,
            date: ts(y, m, Math.min(28, 4 + int(0, 18))),
            method,
            category: 'Last Respect Insurance premium',
            allocations: [{ type: 'welfare', referenceType: 'member_contributions', referenceId: mcId, period, amount: premiumPaid }],
            recordedBy: method === 'cash' ? 4 : null,
            notes: `${PERIOD_LABEL[period]} Last Respect Insurance premium`,
          });
        }
      }
    }
    logger.log('[demo-seed] Last Respect Insurance: ' + enrolled.length + ' members enrolled');
  }
  ...
  const welfareData = [
    {
      id: 1, case_no: 'WEL/2026/0001', member: 9, category: 'education', nature: 'School fees support for two children after the family income earner fell ill',
      hospital: null, ward: null, admission: null, target: 60000, per: 300,
      opening: date(2026, 6, 13), deadline: date(2026, 7, 13), status: 'disbursed', disbursed: 52000, disbursedAt: ts(2026, 7, 15),
    },
    {
      id: 2, case_no: 'WEL/2026/0002', member: 21, category: 'accident', nature: 'Household property destroyed in a fire accident — support to replace essential items',
      hospital: null, ward: null, admission: null, target: 120000, per: 500,
      opening: date(2026, 8, 3), deadline: date(2026, 9, 3), status: 'open', disbursed: 0, disbursedAt: null,
    },
    {
      id: 3, case_no: 'WEL/2026/0003', member: 34, category: 'sickness', nature: 'Outpatient treatment and medication support',
      hospital: null, ward: null, admission: null, target: 25000, per: 200,
      opening: date(2026, 8, 21), deadline: date(2026, 9, 20), status: 'open', disbursed: 0, disbursedAt: null,
    },
  ];
  const welfareCollected = { 1: 0, 2: 0, 3: 0 };
  let wpId = 0;
  ...
  funeralData.forEach((c) => {
    ...
    funeralCases.add({
      ...
      notes: c.notes || 'CMA delegation attended the funeral and a wreath was laid on behalf of the association.',
  ...
  const weddingData = [
    { id: 1, case_no: 'WED/2026/0001', member: 16, spouse: 'Grace Whitmore', wedding: date(2026, 7, 18), venue: 'St. Joseph Mukasa Parish Church', per: 500, target: 30000, deadline: date(2026, 7, 15), status: 'disbursed', disbursed: 24000 },
    { id: 2, case_no: 'WED/2026/0002', member: 41, spouse: 'Mary O’Connor', wedding: date(2026, 10, 10), venue: 'St. Francis of Assisi Sub-Parish, Soweto', per: 500, target: 30000, deadline: date(2026, 10, 5), status: 'open', disbursed: 0 },
  ];
  const weddingCollected = { 1: 0, 2: 0 };
  let wedpId = 0;
  ...
  const projectCollected = { 1: 0, 2: 0, 3: 0, 4: 0 };
  let pcId = 0;
  ...
  function amortise(principal, monthlyPct, months, method, firstDue) {
  ...
  const loanSeed = [
    ...
    { app: 2, no: 'LA/2026/0002', member: 11, type: 'DEV', amount: 350000, months: 36, purpose: 'Completion of a rental unit at Riverdale', status: 'disbursed', applied: ts(2026, 4, 12), loanNo: 'LN/2026/0002', disbursed: ts(2026, 5, 6), guarantors: [13, 14, 15], firstDue: new Date(2026, 5, 10) },
    ...
    { app: 8, no: 'LA/2026/0008', member: 47, type: 'DEV', amount: 600000, months: 48, purpose: 'Land purchase at Westbrook Road', status: 'rejected', applied: ts(2026, 7, 20), loanNo: null, disbursed: null, guarantors: [48, 49], firstDue: null, rejection: 'Requested amount exceeds three times the applicant’s savings; insufficient share capital.' },
  ];
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    meetings.add({
      ...
      start_time: '10:00:00',
      end_time: '12:30:00',
      venue: type === 'general_assembly' ? 'Parish Pastoral Hall' : 'St. Joseph Mukasa Parish Church Hall',
      parish_id: 1,
      church_id: 1,
      chairperson: 'Mr. Robert Ashford',
      secretary: 'Mr. Samuel Rossi',
  ...
  addNotif({ title: 'New welfare case: WEL/2026/0003', body: 'A member needs welfare support with outpatient treatment and medication. A contribution of KSh 200 per member is requested by 20 September 2026.', category: 'welfare', priority: 'high', channels: ['in_system', 'sms'], link: '/welfare/3', created_at: ts(2026, 8, 21), member_id: 2 });
  ...
  notices.add({ id: 4, title: 'Last Respect Insurance — enrolment open', body: 'Members can enrol in the Last Respect Insurance scheme (KSh 200 per month). The scheme pays a cash benefit of KSh 150,000 towards funeral and end-of-life expenses within 48 hours of claim submission, ensuring your loved ones are not burdened during difficult times. Contact the CMA welfare desk to enrol.', category: 'welfare', audience: 'all', parish_id: 1, pinned: true, publish_from: ts(2026, 1, 6), published_by: 2, status: 'published', created_at: ts(2026, 1, 6) });
  ...
  addAudit({ user_id: 1, user_name: 'Mr. Gregory Simmons', action: 'system.initialised', entity_type: 'system', entity_id: null, entity_label: 'CMA System', description: 'System initialised for St. Joseph Mukasa Parish, Kahawa West', old_values: null, new_values: null, ip_address: '41.90.112.14', user_agent: 'Mozilla/5.0', severity: 'info', created_at: ts(2026, 1, 4, 8, 0) });
  addAudit({ user_id: 2, user_name: 'Mr. Daniel Whitfield', action: 'member.created', entity_type: 'member', entity_id: 64, entity_label: 'CMA/SJM/0064', description: 'Registered new CMA member', old_values: null, new_values: { membership_no: 'CMA/SJM/0064' }, ip_address: '41.90.112.20', user_agent: 'Mozilla/5.0', severity: 'info', created_at: ts(2026, 2, 12, 11, 20) });
  addAudit({ user_id: 4, user_name: 'Mr. Edward Callahan', action: 'payment.recorded', entity_type: 'payment', entity_id: 1, entity_label: 'CMA/202509/0001', description: 'Recorded monthly contribution payment', old_values: null, new_values: { amount: 200, method: 'mpesa' }, ip_address: '41.90.112.31', user_agent: 'Mozilla/5.0', severity: 'info', created_at: ts(2025, 9, 5, 15, 10) });
  addAudit({ user_id: 7, user_name: 'Mr. Francis Delaney', action: 'loan.status_approved', entity_type: 'loan_application', entity_id: 3, entity_label: 'LA/2026/0003', description: 'Loan committee approved KSh 30,000 emergency loan', old_values: { status: 'committee_review' }, new_values: { status: 'approved' }, ip_address: '41.90.112.44', user_agent: 'Mozilla/5.0', severity: 'warning', created_at: ts(2026, 7, 14, 16, 40) });
  addAudit({ user_id: 6, user_name: 'Mr. Vincent Hartley', action: 'loan.disbursed', entity_type: 'loan', entity_id: 3, entity_label: 'LN/2026/0003', description: 'Disbursed KSh 30,000 to member', old_values: null, new_values: { principal: 30000 }, ip_address: '41.90.112.50', user_agent: 'Mozilla/5.0', severity: 'warning', created_at: ts(2026, 7, 15, 10, 5) });
  addAudit({ user_id: 5, user_name: 'Mr. Samuel Rossi', action: 'member.status_updated', entity_type: 'member', entity_id: 61, entity_label: 'CMA/SJM/0061', description: 'Membership status changed to suspended', old_values: { membership_status: 'active' }, new_values: { membership_status: 'suspended' }, ip_address: '41.90.112.61', user_agent: 'Mozilla/5.0', severity: 'warning', created_at: ts(2026, 6, 20, 9, 30) });
  addAudit({ user_id: 8, user_name: 'Mr. Bernard Whitaker', action: 'report.exported', entity_type: 'report', entity_id: null, entity_label: 'Loan portfolio', description: 'Exported loan portfolio report to Excel', old_values: null, new_values: null, ip_address: '41.90.112.72', user_agent: 'Mozilla/5.0', severity: 'info', created_at: ts(2026, 8, 30, 14, 0) });

  /* ---------------- member documents (placeholder PDF files) ---------------- */
  logger.log('[demo-seed] generating member document files…');
  ...
  for (const m of docMembers) {
    ...
    const chosen = [{ type: 'national_id', title: 'National ID' }];
    ...
    for (const d of chosen) {
      ...
      try {
        ...
        logger.log(`[demo-seed] could not write ${rel}: ${err?.message}`);
      }
      memberDocs.add({
        id: docId,
        member_id: m.id,
  ...
  /* ---------------- write everything ---------------- */
  logger.log('[demo-seed] writing tables…');
  ...
  logger.log('[demo-seed] summary:', counts.rows[0]);
  logger.log(`[demo-seed] done in ${((Date.now() - t0) / 1000).toFixed(1)}s`);
  logger.log('[demo-seed] demo logins (password in brackets):');
  staff.forEach((s) => {
    const u = users.rows.find((x) => x.id === s.id);
    logger.log(`  ${s.role.padEnd(14)} login: ${String(u.login_id).padEnd(13)} email: ${String(u.email).padEnd(26)} phone: ${u.phone}  [${s.pw}]`);
  });
  logger.log(`  member         ${memberRows[5].phone} (CMA/SJM/0006)  [Member@2026]`);
  logger.log(`  member         ${memberRows[8].phone} (CMA/SJM/0009)  [Member@2026]`);
